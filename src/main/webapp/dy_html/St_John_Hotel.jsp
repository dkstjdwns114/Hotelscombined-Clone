...
try{
	...
	if(conn != null){
		sql = "SELECT * FROM tb_hotel WHERE h_idx=2";
...
try{
	...
	conn = DriverManager.getConnection(url, uid, upw);
	if(conn != null){
		sql = "SELECT * FROM tb_grade WHERE g_idx=2";
		pstmt = conn.prepareStatement(sql);
		rs = pstmt.executeQuery();
...
try{
	...
	conn = DriverManager.getConnection(url, uid, upw);
	if(conn != null){
		sql = "SELECT * FROM tb_facility WHERE f_idx=2";
		pstmt = conn.prepareStatement(sql);
		rs = pstmt.executeQuery();
...
ArrayList<HashMap<String,String>> St_JohnList= detailDao.getSt_JohnList(); request.setAttribute("St_JohnList", St_JohnList);
%>
<c:set var="skybayList" value="${St_JohnList}"/>
...
<c:set var="checkIn" value="<%=searchInfo.getDatepicker1()%>"/>
	...
	<c:set var="location" value="강릉, 대한민국"/>
	</c:when>
                    ...
                    <div class="search_form_details">
                        <form class="searchform" name="sarchform" method="post" action="./find.jsp" autocomplete ="off" onsubmit="return sendit()">
                            ...
                            <div class="destination search_location">
                                ...
                                <div id="search_location"><input type="text"  id="locationText" name="location" value="강릉, 대한민국"></div>
                            ...
                            <div>
                                <div class="checkIn">
                                    ...
                                    <div class="detail_date">
                                        ...
                                        <input type="text" id="datepicker1" class="date_picker" value="${checkIn}">
                                        </div>
                                </div>
                                <div class="checkout">
                                    ...
                                    <div class="detail_date">
                                        <div class="icon"><i class="far fa-calendar-alt fa-lg"></i></div>
                                        <input type="text" id="datepicker2" class="date_picker" value="${checkOut}">
                                    </div>
                                </div>
                                </div>
                                <div class="detail_room roomsGuests">
                                    ...
                                    <div id="roomGuest" class="detail_room"><input id="room_guest" type="text" name="room_guest" value="객실 ${rooms}개, 투숙객 ${adults+chilren}명"></div>
                        ...
                        <section>
                            ...
                            <div class="content">
                                <div class="main_content">
                                    ...
                                    <div class="bxslider">
                                        <img class="main_content_3" src="../xm_images/<%=h_img%>1.jpg">
                                        <img class="main_content_4" src="../xm_images/<%=h_img%>1.jpg">
                                    </divcl>
                            ...
                            <div class="products">
                                ...
                                <div class="product_main">
                                    ...
                                    <div class="hotel_details">
                                        <div class="hotelRoom_name"><a href="../jg_html/payIn_step1.jsp?param=2"> <%=priceDAO.getDesc()[2] %> </a>
                                        </div>
                                        <span><i class="fas fa-check"></i>호텔에서 결제 
                                            • 무료 와이파이 </span>
                                    					...
                                    					<% if(userid == null){%>
														  ...
														  %>
                                                          <div class="hotel_price"><a  href="../jg_html/payIn_step1.jsp?param=12"><%=priceDAO.getOurPrice()[2] %>원</a></div>
                                                        <% } %>
                                    <div class="hotel_btn"><input onclick="location.href='../jg_html/payIn_step1.jsp?param=2'" type="button" value="호텔에서 결제" ></div>
                                </div>
                                 <c:forEach items="${skybayList}" var="map" varStatus="status" begin="0" end="2" step="1" >
                                <c:if test="${status.index eq status.index}">
                                   <c:choose>
                                        <c:when test="${map['logo'] eq 'agoda'}">
                                        <c:url var="url" value="https://www.agoda.com/ko-kr/search?cid=1617473&currency=KRW&checkin=${checkIn}&checkout=${checkOut}&NumberofAdults=${adults}&NumberofChildren=${children }&Rooms=${rooms}&mcid=3038&tag=3541400,pid1334087618&selectedproperty=3541400&city=19041&adults=${adults}&children=${children}&hc=KRW&los=1"/>
                                        </c:when>
                                        <c:when test="${map['logo'] eq 'hotelpass'}">
                                        <c:url var="url" value="http://hotelscombined.hotelpass.com/search/detail.asp?cmd=infoHotel&sNatCtyCode=KRKAG&hoteltype=2&hotelCode=KRKAG011&checkInDate=${checkIn}&checkOutDate=${checkOut}&RoomPassen=${adults},${children},_/&mAgentCD=A000002227&ConversionID=1334089323&NaverYN=N"/>
                                        </c:when>
                                        <c:when test="${map['logo'] eq 'hotels'}">
                                        <c:url var="url" value="https://kr.hotels.com/web/ho655304448?pos=HCOM_KR&locale=ko_KR&q-rooms=${rooms}&q-room-0-adults=${adults}&q-room-0-children=${children}&q-check-in=${checkIn}&q-check-out=${checkOut}&cur=KRW&wapa2=655304448&rffrid=mdp.hcom.KR.112.158.02.kwrd=1334088876"/>
                                        </c:when>
                                        </c:choose>
                                ...
                                <div class="product_main">
                                    ...
                                    <div class="hotel_details ">
                                        <div class="hotelRoom_name"><a href="${url}"> ${map['desc2']} </a>
                                        </div>
                                        <span><i class="fas fa-check"></i>호텔에서 결제 
                                            • 무료 와이파이 </span>
                                    </div>
                                    <div class="hotel_price"><a href="${url}">${map['price2']}원</a></div>
