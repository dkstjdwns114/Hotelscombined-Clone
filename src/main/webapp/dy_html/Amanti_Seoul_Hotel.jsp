...
<jsp:useBean class="com.hotelsCombined.dy_DAO.GetPriceDAO" id="dao"/>
<jsp:useBean class="com.hotelsCombined.dy_DAO.FindDetailDAO" id="detailDao"/>
...
if(searchInfo.getDatepicker1() == null){
	request2.setAttribute("filter_checkin", currentDate);
	request2.setAttribute("filter_checkout", tomorrowDate);
	request2.setAttribute("filter_adult", "1");
	request2.setAttribute("filter_child", "0");
	request2.setAttribute("filter_room", "1");
}else{
...
try{ 
	...
	if(conn != null){
		sql = "SELECT * FROM tb_hotel WHERE h_idx=12";
...
try{
	...
	conn = DriverManager.getConnection(url, uid, upw);
	if(conn != null){
		sql = "SELECT * FROM tb_grade WHERE g_idx=12";
		pstmt = conn.prepareStatement(sql);
		rs = pstmt.executeQuery();
...
try{
	...
	conn = DriverManager.getConnection(url, uid, upw);
	if(conn != null){
		sql = "SELECT * FROM tb_facility WHERE f_idx=12";
		pstmt = conn.prepareStatement(sql);
		rs = pstmt.executeQuery();
...
ArrayList<HashMap<String,String>> amantiList= detailDao.getAmantiList(); request.setAttribute("amantiList", amantiList);

%>
<c:set var="amantiList" value="${amantiList}"/>
...
<body>
    ...
    <div id="wrap">
        ...
        <div class="container">
            ...
            <div class="back_btn"><a href="location.href='history.back();'">&lt;</a> <a href="location.href='history.back();'">뒤로</a></div>
            <div class="left_rail">
                <div class="left_box">
                    <div class="search_form_details">
                        <form class="searchform" name="sarchform" method="post" action="./find.jsp" autocomplete ="off" onsubmit="return sendit()">
                            ...
                            <div class="destination search_location">
                                ...
                                <div id="search_location"><input type="text" name="location" value="서울, 대한민국" id="locationText"></div>
                            ...
                            <div>
                                <div class="checkIn">
                                    ...
                                    <div class="detail_date">
                                        ...
                                        <input type="text" name="datepicker1" id="datepicker1" class="date_picker" value="${checkIn}">
                                        </div>
                                </div>
                                <div class="checkout">
                                    ...
                                    <div class="detail_date">
                                        <div class="icon"><i class="far fa-calendar-alt fa-lg"></i></div>
                                        <input type="text" name="datepicker2" id="datepicker2" class="date_picker" value="${checkOut}">
                                    </div>
                                </div>
                                </div>
                                <div class="detail_room roomsGuests">
                                    ...
                                    <div id="roomGuest" class="detail_room"><input id="room_guest" name="room_guest" type="text" value="객실 ${rooms}개, 투숙객 ${adults+chilren}명"></div>
                            ...
                            <div class="content">
                                <div class="main_content">
                                    ...
                                    <div class="bxslider">
                                        ...
                                        <img class="main_content_23" src="../xm_images/<%=h_img%>1.jpg">
                                        <img class="main_content_24" src="../xm_images/<%=h_img%>1.jpg">
                                    </div>
                ...
                <div class="total_product">
                    <section>
                        <div class="total_top">
                            <div class="clear">
                                ...
                                <div class="total_top_date"> 
                                    ...
                                    <span>${rooms}박, 투숙객 ${adults+children}명</span>
                                    <span>${checkIn}</span>
                                    <span> <i class="fas fa-arrow-right"></i> ${checkOut}</span>
                                    <span><a href="#search_location">변경</a></span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="hotel_products">
                            <div class="products">
                                <div class="product_cnt">예약 가능한 상품: <span>..</span></div>
                                ...
                                <div class="product_main">
                                    ...
                                    <div class="hotel_details">
                                        <div class="hotelRoom_name"><a href="../jg_html/payIn_step1.jsp?param=12"> <%=priceDAO.getDesc()[12] %> </a>
                                        </div>
                                        <span><i class="fas fa-check"></i>호텔에서 결제 
                                            • 무료 와이파이 </span>
                                       					 ...
                                       					 <% if(userid == null){%>
												           <div class="hotel_price"><a href="#"><i class="fas fa-user-lock fa-lg lockedicon"></i></a></div>
												          <%
														  }else{
														  %>
                                                          <div class="hotel_price"><a  href="../jg_html/payIn_step1.jsp?param=12"><%=priceDAO.getOurPrice()[12] %>원</a></div>
                                                        <% } %>
                                            
                                    <div class="hotel_btn"><input onclick="location.href='../jg_html/payIn_step1.jsp?param=12'" type="button" value="호텔에서 결제" ></div>
                                </div>
                                <c:forEach items="${amantiList}" var="map" varStatus="status" begin="0" end="2" step="1" >
                                <c:if test="${status.index eq status.index}">
                                       <c:choose>
                                        <c:when test="${map['logo'] eq 'agoda'}">
                                        <c:url var="url" value="https://www.agoda.com/ko-kr/search?cid=1617473&currency=KRW&checkin=${checkIn}&checkout=${checkOut}&NumberofAdults=${adults}&NumberofChildren=${children }&Rooms=${rooms}&mcid=332&tag=1197749,pid1334088386&selectedproperty=1197749&city=14690&adults=${adults}&children=${children}&hc=KRW&los=1"/>
                                        </c:when>
                                        <c:when test="${map['logo'] eq 'hotelpass'}">
                                        <c:url var="url" value="http://hotelscombined.hotelpass.com/search/detail.asp?cmd=infoHotel&sNatCtyCode=KRSEL&hoteltype=2&hotelCode=KRSEL383&checkInDate=${checkIn}&checkOutDate=${checkOut}&RoomPassen=${adults},${children},_/&mAgentCD=A000002227&ConversionID=1334088580&NaverYN=N"/>
                                        </c:when>
                                        <c:when test="${map['logo'] eq 'hotels'}">
                                        <c:url var="url" value="https://kr.hotels.com/web/ho600060?pos=HCOM_KR&locale=ko_KR&q-rooms=${rooms}&q-room-0-adults=${adults}&q-room-0-children=${children}&q-check-in=${checkIn}&q-check-out=${checkOut}&cur=KRW&wapa2=600060&rffrid=mdp.hcom.KR.112.158.02.kwrd=1334090043"/>
                                        </c:when>
                                        <c:when test="${checkIn eq null}">
                                        <c:url var="url" value="${map['url3']}"/>
                                        </c:when>
                                        </c:choose>
                                ...
                                <div class="product_main">
                                    ...
                                    <div class="hotel_site"><a href="${url}"><img src="../dy_images/${map['logo']}_logo.png" ></a>
                                    </div>
                                    <div class="hotel_details ">
                                        <div class="hotelRoom_name"><a href="${url}"> ${map['desc3']} </a>
                                        </div>
                                        <span><i class="fas fa-check"></i>호텔에서 결제 
                                            • 무료 와이파이 </span>
                                    </div>
                                    <div class="hotel_price"><a href="${url}">${map['price3']}원</a></div>
                                    <div class="hotel_btn_2"><input onclick="location.href='${url}'" type="button" value="바로 예약">
                                    </div>
                                </div>
                                </c:if>
                                </c:forEach>
